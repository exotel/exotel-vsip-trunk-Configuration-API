...
test_go() {
    ...
    # Test each script
    run_test "create_trunk" "cd '$go_dir' && go run exotel_client.go create_trunk.go" "go"
    
    if [ "${test_results[go_create_trunk]}" = "PASSED" ]; then
        run_test "map_did" "cd '$go_dir' && go run exotel_client.go map_did.go" "go"
        run_test "whitelist_ip" "cd '$go_dir' && go run exotel_client.go whitelist_ip.go" "go"
        run_test "add_destination_udp" "cd '$go_dir' && go run exotel_client.go add_destination_udp.go" "go"
        run_test "add_destination_tcp" "cd '$go_dir' && go run exotel_client.go add_destination_tcp.go" "go"
        run_test "add_destination_tls" "cd '$go_dir' && go run exotel_client.go add_destination_tls.go" "go"
        run_test "set_trunk_alias" "cd '$go_dir' && go run exotel_client.go set_trunk_alias.go" "go"
    else
