...
class ExotelAPITester:
    ...
    def make_json_request(
        ...
        """JSON request for GET, POST, PUT, DELETE (Basic auth)."""
        if not self.base_url:
        ...
        self.logger.info(f"Testing {test_name}: {method} {endpoint}")
        try:
            with urllib.request.urlopen(req, timeout=30, context=ssl_context) as response:
            ...
            self.logger.error(f"❌ {test_name} - HTTP {e.code}: {err_body}")
            return False, {"status_code": e.code, "details": err_body}
        except Exception as e:
    ...
    def test_list_trunks(self) -> Dict[str, Any]:
        """GET /trunks (pagination optional)."""
        q = {}
        if os.getenv("PAGE_SIZE"):
            q["page_size"] = os.getenv("PAGE_SIZE")
        if os.getenv("PAGE_OFFSET"):
            q["offset"] = os.getenv("PAGE_OFFSET")
        success, result = self.make_json_request("GET", "/trunks", "List trunks", query=q if q else None)
        test_result = {
            "test_name": "list_trunks",
            "endpoint": "/trunks",
            "method": "GET",
            "success": success,
            "result": result,
    ...
    def test_create_credentials(self, trunk_sid: str = None) -> Dict[str, Any]:
        """POST SIP digest credentials (optional if SIP_CRED_PASSWORD set)."""
        if not trunk_sid:
            trunk_sid = os.getenv("TRUNK_SID", "")
        pwd = os.getenv("SIP_CRED_PASSWORD")
        if not pwd:
            r = {
                "test_name": "create_credentials",
                "skipped": True,
                "reason": "SIP_CRED_PASSWORD not set",
                "timestamp": datetime.datetime.now().isoformat(),
            }
        ...
        if not trunk_sid:
            ...
            return r
        payload = {
            "user_name": os.getenv("SIP_CRED_USERNAME", "test_voice_user"),
            "password": pwd,
            "friendly_name": os.getenv("SIP_CRED_FRIENDLY_NAME", "api_test"),
        }
        ep = f"/trunks/{trunk_sid}/credentials"
        success, result = self.make_json_request("POST", ep, "Create SIP credentials", payload=payload)
        test_result = {
            "test_name": "create_credentials",
            "endpoint": ep,
            "method": "POST",
    ...
    def test_list_credentials(self, trunk_sid: str = None) -> Dict[str, Any]:
        if not trunk_sid:
            trunk_sid = os.getenv("TRUNK_SID", "")
        if not trunk_sid:
            r = {"test_name": "list_credentials", "skipped": True, "reason": "No TRUNK_SID"}
            self.test_results.append(r)
            return r
        ep = f"/trunks/{trunk_sid}/credentials"
        success, result = self.make_json_request("GET", ep, "List credentials")
        test_result = {
            "test_name": "list_credentials",
            "endpoint": ep,
            "method": "GET",
            "success": success,
            "result": result,
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.test_create_credentials(trunk_sid)
        self.test_list_credentials(trunk_sid)
...
def main():
    ...
    parser.add_argument("--test", "-t", choices=["create", "map", "whitelist", "destination", "alias", "list_trunks", "credentials", "all"], 
    ...
    if args.test == "all":
        ...
        elif args.test == "list_trunks":
            tester.test_list_trunks()
        elif args.test == "credentials":
            tester.test_create_credentials()
            tester.test_list_credentials()
